...
public class BookServiceImpl implements BookService {
    ...
    public List<BookDTO> getAllBooks() {
        ...
        return books.stream()
                .map(this::convertToDTOWithMongoDB)
    ...
    public BookDTO getBookById(Long id) {
        ...
        // 从MongoDB获取扩展信息
        BookDetail detail = bookDetailRepository.findByBookId(id).orElse(null);
        if (detail != null) {
            dto.setDescription(detail.getDescription());
            dto.setCover(detail.getCover());
        }
    ...
    public List<BookDTO> searchBooksByTag(String tagName) {
        // 1. 从Neo4J获取与指定标签相关的所有标签（包括2度关联）
        Set<String> relatedTags = tagService.findRelatedTagNames(tagName);
        
        // 2. 在MySQL中搜索所有带有这些标签中任意一个的图书
        List<Book> allBooks = new ArrayList<>();
        Set<Long> bookIds = new HashSet<>(); // 用于去重
        
        for (String tag : relatedTags) {
            List<Book> books = bookRepository.searchBooksByTag(tag);
            for (Book book : books) {
    ...
    public Book saveBook(Book book) {
        ...
        // 如果book有description或cover，同步保存到MongoDB
        if (book.getDescription() != null || book.getCover() != null) {
            BookDetail detail = bookDetailRepository.findByBookId(savedBook.getId()).orElse(new BookDetail());
            detail.setBookId(savedBook.getId());
    ...
    /**
     * 转换为DTO并填充MongoDB中的扩展信息
     */
    private BookDTO convertToDTOWithMongoDB(Book book) {
        BookDTO dto = convertToDTO(book);
        
        // 从MongoDB获取扩展信息
        BookDetail detail = bookDetailRepository.findByBookId(book.getId()).orElse(null);
        if (detail != null) {
