...
public class BookController {
    ...
    public ResponseEntity<Map<String, Object>> searchBooksByTag(@RequestParam String tag) {
        Map<String, Object> response = new HashMap<>();
        try {
            ...
            System.out.println("根据标签搜索书籍失败: " + e.getMessage());
            e.printStackTrace();
            response.put("success", false);
            response.put("message", "根据标签搜索书籍失败：" + e.getMessage());
            return ResponseEntity.ok(response);
        }
    }
} 