...
export const bookApi = {
  ...
  // 根据标签搜索书籍
  searchBooksByTag: (tag) => request(`/books/search/by-tag?tag=${encodeURIComponent(tag)}`),
};
...
export const tagApi = {
  ...
  // 获取相关标签（包括2度关联）
  getRelatedTags: (tagName) => request(`/tags/related?tagName=${encodeURIComponent(tagName)}`),
  
  // 初始化标签图（管理员功能）
  initializeTags: () => request('/tags/initialize', {
    method: 'POST',
  }),
};
